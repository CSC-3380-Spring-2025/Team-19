...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: "Scattergories Leaderboard",
      theme: ThemeData(
        primarySwatch: Colors.deepPurple,
        scaffoldBackgroundColor: Colors.deepPurple[50],
      ),
      home: ConnectionsLeaderboard(),
    );
  }
}

class ConnectionsLeaderboard extends StatefulWidget {
  @override
  _ConnectionsLeaderboardPageState createState() => _ConnectionsLeaderboardPageState();
}

class _ConnectionsLeaderboardPageState extends State<ConnectionsLeaderboard> {
  ...
  Future<List<LeaderboardEntry>> fetchLeaderboard() async {
    ...
    for (User user in users) {
      if (user.connectionsTimes.containsKey(dateID)) {
        entries.add(
          LeaderboardEntry(username: user.name, timeTaken: user.connectionsTimes[dateID]!, score: user.connectionsScores[dateID]!),
        );
      }
    }
